...
def canny(kernel_size, standard_deviation, bound_part, operator_mode, path):
    ...
    # make gray img
    frame = cv2.imread(f"{dir_path}/imgs/" + path, cv2.IMREAD_GRAYSCALE)
    ...
    isCompleted = cv2.imwrite(f"{dir_path}/output/" + file_name, img_border_filtered)
    if isCompleted:
...
def laplacian_of_gaussian(kernel_size, standard_deviation, bound, path):
    ...
    # make gray img
    frame = cv2.imread(r".\imgs\\" + path, cv2.IMREAD_GRAYSCALE)
